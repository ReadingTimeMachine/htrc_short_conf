...
plot_diagnostics = False # toggle on if you want diagnostic plots -- generally slows things down!
...
save_binary_dir = '/Users/jnaiman/Dropbox/wwt_image_extraction/FigureLocalization/StoredFeatures/MegaYolo_htrc/' # where stored features will be (eventually)
...
generate_features = False # this is depricated and now replaced with the "generate_features.py" script
...
for sto, iw in yt.parallel_objects(wsInds, config.nProcs, storage=my_storage):
    ...
    if os.path.exists(pdfExpected) and use_pdfmining:
        ...
        if use_pdfmining:
            ...
            print('cant find PDF where it should be:', pdfExpected)
